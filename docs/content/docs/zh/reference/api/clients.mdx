...
_openapi:
  ...
  toc:
    ...
    - depth: 2
      title: >-
        在一次调用中创建一个新客户端并将其挂载到一个或多个入站。请求体为 JSON。省略时会生成
        各协议凭据：VLESS/VMess 的 UUID、Trojan/Shadowsocks 的 password、Hysteria 的
        auth、MTProto 的 secret 和 WireGuard 密钥对。仅提供 WireGuard 私钥时会派生公钥。
        省略 allowedIPs 时，会从目标入站的对等子网分配地址；若没有剩余地址，分配可能失败。
      url: >-
    ...
    - depth: 2
      title: >-
        将现有客户端挂载到一个或多个额外的入站。请求体为 JSON。对于 WireGuard，如果目标
        入站中的另一个客户端已使用现有 allowedIPs 值，则挂载会被拒绝。
      url: >-
